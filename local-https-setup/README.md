...
3. **Enable local DNS** (first time only):
   ```bash
   ./local-https-setup/toggle-local-dns.sh
   ```
4. **Update config.js from the test env for all defra id related changes** :
...
3. Regenerate certificates: Delete existing `.pem` files and restart
