...
export const getActionButtons = (project) => {
  ...
  if (isOwnProject) {
    if (project.status === 'Draft') {
      return `<a href="${routes.TASK_LIST}/${project.id}" class="govuk-link govuk-!-margin-right-4 govuk-link--no-visited-state" aria-label="Continue to task list">Continue</a><a href="${routes.DELETE_EXEMPTION}/${project.id}" class="govuk-link govuk-link--no-visited-state" aria-label="Delete ${project.projectName}">Delete</a>`
    }
    return `<a href="${routes.VIEW_DETAILS}/${project.id}" class="govuk-link govuk-link--no-visited-state" aria-label="View details of ${project.projectName}">View details</a>`
  }
...
export const formatProjectsForDisplay = (projects, isEmployee = false) =>
  projects.map((project) => {
    ...
    baseRow.push({ html: getActionButtons(project, isEmployee) })
