...
import { ADDRESS_HEADING, ADDRESS_TYPE_HEADING, CONTACT_EMAIL, CONTACT_FULL_NAME, CONTACT_ORG_NAME, CONTACT_PHONE_NUMBER, PO_HEADING } from '#src/server/common/helpers/marine-licence/invoicing/invoicing-review-data.js'
...
describe('Marine Licence Invoicing Card Component', () => {
  ...
  test('Should show organisation name and PO number when isIndividual is false', () => {
    const $comp = renderComponent(
      'marine-licence/invoicing-card',
      {
        invoicingData,
    ...
    expect($comp.html()).toContain(CONTACT_ORG_NAME)
    expect($comp.html()).toContain('Example Organisation')
    expect($comp.html()).toContain(PO_HEADING)
    expect($comp.html()).toContain('PO-12345')
  })
